...
def plot_pdf_empirical():
    ...
    bins1 = torch.load("data/cvs/xi_1_bins.pt")
    bins2 = torch.load("data/cvs/xi_2_bins.pt")

    fig, ax = plt.subplots(figsize=(2, 2.25), layout="constrained")
    pcm = ax.pcolormesh(
        bins1, bins2, pdf.T, norm=mpl.colors.LogNorm(1e-5, 1e0), rasterized=True
    )
    ax.set_xlabel("$\\xi_1$")
    ax.set_ylabel("$\\xi_2$")
    fig.colorbar(pcm, ax=ax, label=r"$\pi$", location="top")
...
def plot_rate_heatmap():
    all_features = ["CVs", "Dihedrals"]
    scores = ["VCN", "EVCN"]
    lags = [1, 100, 10000]
    lag_labels = ["1 ps", "100 ps", "10 ns"]
    two_lags = [(1, 100), (1, 10000), (100, 10000)]
    two_lag_labels = ["1 ps, 100 ps", "1 ps, 10 ns", "100 ps, 10 ns"]
