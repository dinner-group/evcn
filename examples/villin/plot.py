...
def plot_pdf_empirical():
    ...
    bins1 = torch.load("data/cvs/RMSD_12_bins.pt")
    bins2 = torch.load("data/cvs/RMSD_23_bins.pt")

    fig, ax = plt.subplots(figsize=(2, 2.75), layout="constrained")
    pcm = ax.pcolormesh(
        bins1, bins2, pdf.T, norm=mpl.colors.LogNorm(1e-2, 1e2), rasterized=True
    )
    ax.set_xlabel("RMSD$_{12}$ (nm)")
    ax.set_ylabel("RMSD$_{23}$ (nm)")
...
def plot_histogram(features):
    ...
    fig, axes = plt.subplots(
        3, 2, figsize=(4, 6.5), layout="constrained", sharex=True, sharey=True
    )
